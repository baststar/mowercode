...
void ADCManager::printCalib() {
    ...
    switch (sampleRate) {
    case SRATE_38462:
        Serial.println(F("38462"));
        break;
    case SRATE_19231:
        Serial.println(F("19231"));
        break;
    case SRATE_9615:
        Serial.println(F("9615"));
        break;
    }
    for (int ch = 0; ch < CHANNELS; ch++) {
        Serial.print(F("AD"));
...
void ADCManager::startADC(int sampleCount) {
    ...
    if (sampleCount == 1)
        ...
        switch (sampleRate) {
        case SRATE_38462:
            ADCSRA = _BV(ADSC) | _BV(ADEN) | _BV(ADATE) | _BV(ADIE) | _BV(ADPS2) | _BV(ADPS0);
...
void ADC_Handler(void) {
    ...
    // Serial.println(String(position) + " " + String(captureSize[channel]));
    value -= ofs[channel];
