...
void Manouver_Mow_The_Grass()
{
    ...
    if ((Pattern_Mow == 2) && (Spiral_Mow == 3))
        Max_Cycles_Active = Max_Cycles_Straight; // if spiral is selected but its a straight line leg
    if ((Pattern_Mow == 2) && (Spiral_Mow < 3))
        Max_Cycles_Active = Max_Cycles_Spirals; // if spiral is selected and its a LH or RH spiral
...
void Manouver_Find_Wire_Track()
{
    ...
    for (int i = 0; i <= 1; i++) {
        if (Mower_Parked == 0) {
            ...
            if ((inside == false) && (Abort_Wire_Find == 0)) { // If the mower is outside the wire then run the following code.
                ...
                while ((inside != true) && (Abort_Wire_Find == 0) &&
                       (No_Wire_Found_Bck == 0)) { // While the mower is still outside the fence run this code
            ...
            if ((inside == true) && (Abort_Wire_Find == 0) &&
                (No_Wire_Found_Fwd == 0)) { // If the Mower is situated  the wire then run the following code.
                ADCMan.run();
                UpdateWireSensor();
                ...
                while ((inside != false) && (No_Wire_Found_Fwd == 0) &&
                       (Mower_Parked == 0)) { // Move the mower forward until mower is outisde/ON the wire fence or 500 cycles have passed
    ...
    while ((inside == false) && (Abort_Wire_Find == 0) && (No_Wire_Found_Fwd == 0) && (Mower_Parked == 0) &&
           (Spin_Attempts < Max_Spin_Attempts)) {
