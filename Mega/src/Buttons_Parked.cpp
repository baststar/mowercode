...
{
    if (LCD_Menu_Parked == 1)
        lcd.print("Mow Grass     ");
    if (LCD_Menu_Parked == 2) {
        if (Use_Charging_Station == 1)
            lcd.print("Go To Dock");
        if (Use_Charging_Station == 0)
            lcd.print("Dock Removed");
    }
    if (LCD_Menu_Parked == 3)
        lcd.print(F("Trampoline Cut"));
    if (LCD_Menu_Parked == 4)
        lcd.print(F("Test Menu"));
    if (LCD_Menu_Parked == 5)
        lcd.print(F("Setup Mower"));
    if (LCD_Menu_Parked == 6)
        lcd.print(F("-- SPARE 6 ---"));
    if (LCD_Menu_Parked == 7)
        lcd.print(F("-- SPARE 7 ---"));
    if (LCD_Menu_Parked == 8)
        lcd.print(F("-- SPARE 8 ---"));
    if (LCD_Menu_Parked == 9)
        lcd.print(F("-- SPARE 9 ---"));
    if (LCD_Menu_Parked == 10)
        lcd.print(F("-- SPARE 10 ---"));
    if (LCD_Menu_Parked == 11)
        lcd.print(F("-- SPARE 11 ---"));
    if (LCD_Menu_Parked == 12)
