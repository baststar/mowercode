...
void Get_Compass_Reading2() {
    ...
    // Set declination angle. Find your location declination on:
...
void Compass_Turn_Mower_To_Home_Direction2() {
    // Stop the motors.
...
void Turn_To_Compass_Heading2() {

    Compass_Target = Home_Wire_Compass_Heading;
    Get_Compass_Reading2();
...
void Calculate_Compass_Wheel_Compensation() {
    float Compass_Error = Compass_Heading_Degrees - Heading_Lock; // Calculates the error in compass
                                                                  // heading from the saved lock heading
    ...
    if (Compass_Error < 0) { // Steer left
        Serial.print(F("SR|"));
        PWM_Right = PWM_MaxSpeed_RH + (CPower * Compass_Error); // Multiply the difference by D to increase
                                                                // the power then subtract from the PWM
        if (PWM_Right < 0)
            PWM_Right = PWM_MaxSpeed_RH - 50;
        PWM_Left = PWM_MaxSpeed_LH; // Keep the Right wheel at full power
                                    // calibrated to go straight
    }
    if (Compass_Error >= 0) {
        Serial.print(F("SL|"));
        PWM_Right = PWM_MaxSpeed_RH;                           // Keep the Left wheel at full power
                                                               // calibrated to go straight
        PWM_Left = PWM_MaxSpeed_LH - (CPower * Compass_Error); // Multiply the difference by D to increase
                                                               // the power then subtract from the PWM
        if (PWM_Left < 0)
            PWM_Left = PWM_MaxSpeed_LH - 50;
    }
}
