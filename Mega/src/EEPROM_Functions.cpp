...
void Load_EEPROM_Saved_Data()
{
    ...
    if (Alarm_1_Saved_EEPROM == 1) {
        ...
        if (Alarm_1_ON == 1) {
            Serial.println(" | Alarm 1 Active");
        }
        if (Alarm_1_Action == 1)
            Serial.println("Exit Zone 1");
        if (Alarm_1_Action == 2)
            Serial.println("Exit Zone 2");
        if (Alarm_1_Action == 3)
            Serial.println("Mow the Line");
        if (Alarm_1_Action == 4)
    ...
    if (Pattern_Mow_EEPROM == 1) {
        ...
        if (Pattern_Mow == 0)
            Serial.println(F("OFF"));
        if (Pattern_Mow == 1)
            Serial.println(F("ON Parallel"));
        if (Pattern_Mow == 3)
            Serial.println(F("ON Spiral"));
    }
    ...
    if (Compass_Home_EEPROM == 1) {
        Home_Wire_Compass_Heading = (EEPROM.read(28) * 10); // *10 as value can be more than 255. Vaule is
                                                            // therefore stored as a tenth value
